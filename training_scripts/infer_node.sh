...
if [ "${dataset_name}" = "screenspotpro" ]; then
    val_dataset="/mnt/vlm-ks3/zhangyan/datasets/nips_data/swift_data/ground_benchmark/screenspotpro.jsonl"
...
elif [ "${dataset_name}" = "train" ]; then
    val_dataset="/mnt/vlm-ks3/zhangyan/datasets/nips_data/swift_data/ground_benchmark/train.jsonl" 
fi

# ====== 遍历所有 checkpoint ======
for ckpt in $(ls -1d ${vdir}/checkpoint-*   1 | sort -V); do
...
echo "All checkpoints in ${vdir} evaluated!"
